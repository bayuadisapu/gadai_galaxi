...
class _RegisterPageState extends State<RegisterPage> {
  ...
  // Pilihan cabang untuk registrasi
  ...
  Future<void> _loadBranches() async {
    try {
      final all = await _svc.fetchBranches();
      // Hilangkan cabang 'all' dari pilihan
      final valid = all.where((b) => b.id != 'all').toList();
      if (mounted) {
        setState(() {
          _branches = valid;
          _selectedBranchId = valid.isNotEmpty ? valid.first.id : null;
        });
      }
    } catch (_) {}
  }
  ...
  void _handleRegister() async {
    ...
    try {
      // Gunakan cabang yang dipilih nasabah
      final branchId = _selectedBranchId ?? 'all';
